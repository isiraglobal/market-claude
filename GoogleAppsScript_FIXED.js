...
const CONFIG = {
  ...
  // 🚨 SET TO YOUR VERCEL APP DEPLOYMENT URL (e.g. 'https://your-app.vercel.app/api/sync')
  SYNC_WEBHOOK_URL: 'https://outreach-tan-theta.vercel.app/api/sync',
...
function logStockPrices() {
  ...
  // Trigger Webhook to notify site to pull new data
  if (CONFIG.SYNC_WEBHOOK_URL) {
    try {
      console.log(`[MarketAI] Triggering webhook: ${CONFIG.SYNC_WEBHOOK_URL}`);
      const options = {
        method: 'post',
        contentType: 'application/json',
        payload: JSON.stringify({ source: 'GoogleAppsScript' }),
        muteHttpExceptions: true
      };
      const response = UrlFetchApp.fetch(CONFIG.SYNC_WEBHOOK_URL, options);
      console.log(`[MarketAI] Webhook Response (HTTP ${response.getResponseCode()}): ${response.getContentText()}`);
    } catch (e) {
      console.error(`[MarketAI] Webhook Trigger Error: ${e.message}`);
    }
  }
}
