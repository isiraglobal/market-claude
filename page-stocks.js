Pages.stocks = function(el) {
  ...
  function render() {
    ...
    el.innerHTML = `
    <div class="page-hdr">
      <div><div class="page-title">Stocks</div><div class="page-sub">${syms.length} of ${S.symbols.length} symbols (${S.symbols.length - syms.length} with insufficient data) — NSE</div></div>
  ...
  window.stSearch = v => { q = v; page = 1; const aid = document.activeElement?.id, ss = document.activeElement?.selectionStart, se = document.activeElement?.selectionEnd; render(); if (aid) { const el = document.getElementById(aid); if (el) { el.focus(); if (ss != null && se != null) el.setSelectionRange(ss, se); } } };
  window.stTab = (t, btn) => { tab = t; page = 1; render(); };
  window.stSort = col => { if (sort === col) dir *= -1; else { sort = col; dir = col === 'sym' ? 1 : -1; } page = 1; render(); };
  window.stPage = delta => { page += delta; render(); };
  render();
};
