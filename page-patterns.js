Pages.patterns = function(el) {
  ...
  function render(pats) {
    ...
    window.patQ = v => { q = v; doFilter(); };
    window.patType = v => { filterType = v; doFilter(); };
    window.patScore = v => { minScore = v; document.getElementById('scoreV').textContent = v; doFilter(); };
    window.patReset = () => { filterType = 'all'; filterStatus = 'all'; minScore = 0; q = ''; doFilter(); };
  }
  ...
  el.innerHTML = '<div class="empty"><div class="ai-spinner"><div class="spin-ring"></div>Scanning ' + S.symbols.length + ' symbols for patterns...</div></div>';
