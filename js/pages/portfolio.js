  ...
  function renderPage() {
    ...
    <div class="page-body">
      ...
      <div class="tbl-wrap"><table>
        <thead><tr><th>Symbol</th><th>Qty</th><th>Avg Buy</th><th>Current</th><th class="hide-mobile">Invested</th><th class="hide-mobile">Value</th><th>P&L</th><th>P&L %</th><th class="hide-mobile">Date</th><th></th></tr></thead>
        <tbody>${allPL.map(e => {
          const cls = e.pl && e.pl.pl >= 0 ? 'up' : 'dn'; return `<tr>
          ...
          <td class="hide-mobile">${fp(e.qty * e.avgBuy)}</td>
          <td class="hide-mobile">${e.pl ? fp(e.pl.curV) : '—'}</td>
          ...
          <td style="font-size:10px;color:var(--t3)" class="hide-mobile">${e.date || '—'}</td>
    ...
    window.delPos = async id => { if (!confirm('Remove position?')) return; S.portfolio = S.portfolio.filter(e => e.id !== id); await saveUserData(); renderPage(); toast('Position removed'); };
  }
